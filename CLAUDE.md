...
| `mcp` | `casehub-iot-mcp` | MCP tool surface (`iot_get_devices`, `iot_get_state`, `iot_send_command`, `iot_get_history`). MCP resource surface (`iot://devices/{deviceId}/state`, `iot://devices/changes`) — subscribable device state resources via platform `McpResourceRegistry` SPI with push notifications on `StateChangeEvent`. Library — add with `quarkus-mcp-server-http` to any Quarkus app for LLM agent device access. `@RolesAllowed` enforced when host has a security extension; tenancy-filtered via `McpIdentityContext` with graceful fallback for unsecured hosts (bridge); cross-tenant admins (`isCrossTenantAdmin()`) bypass tenancy filtering. |
...
| `webapp` | `casehub-iot-webapp` | Standalone Quarkus app — operational console with RAS situational awareness, case orchestration, REST API, SSE, TypeScript pages via Quinoa, `IoTAiResolutionAgent` (LLM resolution via `@Scheduled` polling), `IoTCbrReEvaluationObserver` (event-driven CBR re-routing on context changes). `DefaultIoTSimulationApi` (`@McpDomain("iot/simulation")`) for temporal simulation control. Pages scenario runtime with WebSocket `/push` endpoint and bundled scenarios in `META-INF/scenarios/`. Three-datasource Flyway layout. |
